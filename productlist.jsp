					...
					<ul>
						...
						<c:choose>
							...
							<c:when test="${userId ne null && user_where=='finalluser'}">
								...
								<li><a href="cartlist.do?id=${userId}&product_cod=${redisproduct.product_cod}">장바구니</a></li>
...
${redisproduct.product_img}

		<div id="content2">
				
							
			 	<c:if test="${redisproduct.product_Registration_status eq 'open'}">
					<div class="Section2_2">

						<div class="imgarea"
							style="background-image:url('./img_product/${redisproduct.product_img}')"></div>
											<div class="sameinfo">
							<div class="infoarea">
							     <span class="titleinfo">상품명: ${redisproduct.product_name}</span>
								 <span class="detailinof">상품정보: ${redisproduct.product_info}</span>
								 <span class="detailinof">현재 남은 수량 ${redisproduct.product_order_quantity}</span>
							</div>
							...
							<c:choose>
								<c:when test="${userId ne null}">
									...
									<input type="hidden" class="product_cod" name="product_cod"
										value="${redisproduct.product_cod}">
									<input type="hidden" name="product_name"
										value="${redisproduct.product_name}">
									<input type="hidden" name="product_price"
										value="${redisproduct.product_price}">
									<input type="hidden" name="product_img"
										value="${redisproduct.product_img}">
									<!-- <input type="hidden" name="cart_quantity" value="1">  -->
									<div class="btnarea">
										<c:if test="${redisproduct.product_status eq '품절'}">
											<span class="soldout">품절</span>
										</c:if>
										<c:if test="${redisproduct.product_status ne '품절'}">
	...
	/* 	console.log(e.target);
		...
		*/		
		...
		console.log($(this))
		console.log($(this).parent())
	 	console.log("user_code->> "+$(this).parent().prev().prev().prev().prev().prev().prev().val())
		console.log("id->>"+$(this).parent().prev().prev().prev().prev().prev().val())
	console.log("product_cod->>"+$(this).parent().prev().prev().prev().prev().val())
	console.log("product_name->> "+$(this).parent().prev().prev().prev().val())
	console.log("product_price->>"+$(this).parent().prev().prev().val()) 
