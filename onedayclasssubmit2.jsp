...
/*달력시작  */	
   ...
   window.location.replace("/finall/onedayclasspay.jsp?onedayclass_num="+onedayclass_num+"&openday="+openday
		...
		+"&rest="+rest
									...
									window.location.href="/finall/login.jsp";
		...
		$(document).on("click",".nextpagebtn",()=>{	
			...
			$.ajax({
				...
				success:(data)=>{
					...
					console.log("받은데이터 시작")
					
					 console.log(data);
					
					console.log("받은데이터 종료")
...
<p class="onedayclass_info">${first.onedayclass_info}</p>
	 */
					...
					<div class="commentwrapper">
						<h3 class="reviewsize">
							<span>후기: ${joinToReview.size()}건</span>
							
							<script type="text/javascript">
							function 후기작성하기(){
								
								$.ajax({
									url : "paymentandusing.do",												
									method : "GET",
									success : function(val) {
										console.log(val);
										...
										if(val==-1){
											alert("해당 클래스를 이용하신 적이 없습니다.\n 공정한 후기를 위해 원데이 클래스를 이용해주세요!");
											return;
										}
										alert("작성하기")
							...
							<span id="writing" onClick="후기작성하기()">후기 작성</span>
